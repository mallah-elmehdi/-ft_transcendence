import { Avatar, Box, Grid, GridItem, HStack, Text, useMediaQuery, useTheme, VStack } from '@chakra-ui/react';
...
export default function GamePage() {
    ...
    const user_name = 'emallah';
    const user_photo = 'https://cdn.intra.42.fr/users/emallah.jpg';

    const opponent_name = 'aymaatou';
    const opponent_photo = 'https://cdn.intra.42.fr/users/aymaatou.jpg';
    ...
    React.useEffect(() => {
        ...
        const update = () => {
            ...
            if (ball.x + ball.r > getCanvasSize().w) {
                setOpponentScore((data) => data + 1);
                initBall();
            } else if (ball.x - ball.r < 0) {
                setUserScore((data) => data + 1);
                initBall();
            }
    ...
    return (
        <VStack alignContent="center" justifyContent="center">
            <Grid h="100%" templateColumns="repeat(8, 1fr)" gap={3}>
                <GridItem colSpan={4}>
                    <HStack justifyContent="flex-end" spacing={10}>
                        <HStack justifyContent="flex-end" spacing={3}>
                            {isSmallScreen && <Text fontSize="xl">{opponent_name}</Text>}
                            <Avatar name={opponent_name} src={opponent_photo} size="md" />
                        </HStack>
                        <Text fontSize="4xl">{opponentScore}</Text>
                    </HStack>
                </GridItem>
                <GridItem colSpan={4}>
                    <HStack justifyContent="flex-end" spacing={10}>
                        <Text fontSize="4xl">{userScore}</Text>
                        <HStack justifyContent="flex-end" spacing={3}>
                            <Avatar name={user_name} src={user_photo} size="md" />
                            {isSmallScreen && <Text fontSize="xl">{user_name}</Text>}
                        </HStack>
                    </HStack>
                </GridItem>
            </Grid>
